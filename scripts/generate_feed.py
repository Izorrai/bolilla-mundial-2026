...
T = {
    ...
    # Es la que mas dispara en arranques (28 altas) → maxima variedad.
    # Estilo "Alta nueva: X. DCS pendiente de tramitar." con LER, SADER,
    # codigos de tratamiento (D0902, D0901, R1208, CTR D1501/R1301) y HP.
    ("debut", "residuos"): [
        ...
        "{actor} se da de alta. Asignar código LER y a tramitar.",
        "Alta provisional: {actor}. 15 días para completar trámites SADER.",
        ...
        "{actor} llega a almacén. Asignar contenedor según LER.",
        ...
        "{actor}, alta provisional. Falta análisis del LER 200301.",
        "Recepción de {actor}. A coger turno en el muelle de descarga.",
        "{actor} se incorpora. Pendiente formación PRL.",
        # ===== NUEVAS con argot real LER/SADER/D-codes =====
        "Alta nueva: {actor}. LER 130110 por confirmar.",
        "Nuevo productor: {actor}. Pendiente caracterización HP6.",
        "{actor} se da de alta. SADER avisado, esperando código LER.",
        "{actor} entra como nuevo productor. Tipología: oleosos R1208.",
        "Alta de {actor}. Asignar tratamiento a CTR D1501.",
        "{actor} firma alta. Caracterización: residuo peligroso, HP5/HP6.",
        "Nuevo cliente: {actor}. Lodos D0901 por validar.",
        "Llega {actor}. Solidificación-estabilización D0902 prevista.",
        "{actor} estrena ficha. SADER actualizando registro de productores.",
        "Alta confirmada: {actor}. LER pendiente de validar en SADER.",
        "{actor} entra en cartera. Esperando análisis del laboratorio.",
        "Nuevo expediente: {actor}. Ácidos D0901 declarados.",
        "Llegada de {actor}. Emulsiones R1208 en próxima recogida.",
        "Alta provisional de {actor}. Falta confirmar peligrosidad HP.",
        "Nuevo productor: {actor}. Caracterización LER 070108 en marcha.",
        "{actor} firma compromiso. Servicio CTR R1301 asignado.",
        "Recepción de {actor}. Falta acta SADER firmada.",
        "Alta nueva: {actor}. Aceites hidráulicos sintéticos declarados (LER 011311).",
        "{actor}, alta provisional. Pendiente análisis de HP14.",
        "Nuevo en SADER: {actor}. Tipología por confirmar.",
        "{actor} se da de alta. Lodos del tratamiento in situ por catalogar.",
        "Alta de {actor}. CTR D1501 esperando muestras representativas.",
        "{actor} entra. Disolventes organohalogenados a destilación.",
        "Nuevo registro: {actor}. Bidones de RP en recepción.",
        "Alta tramitada: {actor}. Falta inscribir en SADER.",
    ],
    ...
    ("overtake", "residuos"): [
        ...
        "{actor} tramita LER al primer intento. {victim} devuelto a producción.",
        ...
        "{actor} aplica inertización D0902. {victim} a la balsa de lixiviados.",
        ...
        # ===== NUEVAS con argot real =====
        "{actor} tramita LER en SADER antes que {victim}. {victim} todavía en borrador.",
        "{actor} cierra contrato D0902. {victim} pendiente de visita técnica.",
        "{actor} presenta caracterización HP6 en regla. {victim} devuelto a productor.",
        "{actor} clasifica oleoso R1208 al primer intento. {victim} a re-analizar.",
        "Báscula limpia para {actor} (LER 130110). {victim} con tara errónea.",
        "{actor} entrega solidificación D0902. {victim}, lodos sin validar.",
        "{actor} gana licitación CTR. {victim}, propuesta fuera de plazo.",
        "{actor} tramita LER 060101 sin errores. {victim} olvidó el código.",
        "{actor} factura el albarán de oleosos R1208. {victim}, residuo todavía en almacén.",
        "{actor} presenta HP5/HP6 documentado. {victim} en periodo de subsanación.",
        "{actor} envía manifiesto SADER. {victim} rechazado por inspector.",
        "{actor} cierra ácidos D0901 del cliente. {victim} aún esperando trasvase.",
        "{actor} cierra ruta de cisternas. {victim} con conductor pendiente de ADR.",
        "{actor} clasifica emulsión no clorada (LER 130105) correctamente. {victim} mal etiquetada.",
        "{actor} envía lodos D0901 al gestor final. {victim} todavía caracterizando.",
        "{actor} liquida CTR D1501 del trimestre. {victim}, propuesta fuera de plazo.",
        "{actor} actualiza HP del residuo de {victim}. {victim} sin documentar.",
        "{actor} resuelve discrepancia en báscula. {victim} con tara reclamada.",
    ],
    ...
    ("big_up", "residuos"): [
        ...
        "{actor} pasa el LER al primer intento. +{delta}.",
        ...
        "Inertización D0902 exitosa de lote completo: {actor} +{delta}.",
        ...
        # ===== NUEVAS con argot real =====
        "{actor} consigue homologación D0902 nueva. +{delta} puestos.",
        "Auditoría LER superada por {actor} sin incidencias. +{delta}.",
        "{actor} presenta caracterización completa de HP. +{delta}.",
        "Pase limpio en báscula con LER 130110: {actor} +{delta}.",
        "{actor} cierra cartera de oleosos R1208 del trimestre. +{delta}.",
        "{actor} tramita 50 entradas SADER en una semana. +{delta}.",
        "{actor} firma servicio CTR D1501 marco. +{delta}.",
        "Bonificación por gestión ejemplar HP6: {actor} +{delta}.",
        "{actor} presenta memoria SADER antes de plazo. +{delta}.",
        "{actor} resuelve discrepancia HP14 en laboratorio. +{delta}.",
        "{actor} cierra lote completo de ácidos D0901. +{delta}.",
        "{actor} captura cliente nuevo con producción mensual. +{delta}.",
        "Inscripción nueva en SADER tramitada por {actor}. +{delta}.",
    ],
    ...
    ("big_down", "residuos"): [
        ...
        # ===== NUEVAS con argot real =====
        "{actor} olvida tramitar LER en SADER: -{delta}.",
        "Auditoría con incidencias en HP14: {actor} -{delta}.",
        "{actor} devuelve un oleoso R1208 mal clasificado: -{delta}.",
        "Ácidos D0901 sin etiquetar correctamente: {actor} -{delta}.",
        "{actor} presenta acta SADER con errores: -{delta}.",
        "Solidificación D0902 rechazada por composición: {actor} -{delta}.",
        "Cisterna de emulsiones R1208 no etiquetada: {actor} -{delta}.",
        "{actor} olvidó la peligrosidad HP de un LER: -{delta} preventivos.",
        "Sanción SADER por LER mal declarado: {actor} -{delta}.",
        "{actor} mezcla códigos LER incompatibles: -{delta}.",
        "Lodos D0901 sin caracterizar antes del envío: {actor} -{delta}.",
        "{actor} no envía manifiesto a SADER en plazo: -{delta}.",
        "Bidones de RP sin precintar: {actor} -{delta}.",
    ],
    ...
    ("new_leader", "residuos"): [
        ...
        # ===== NUEVAS =====
        "Nuevo jefe de SADER: {actor}. {prev_leader} pasa a relevo de turno.",
        "{actor} consigue homologación CTR del año. {prev_leader}, contrato no renovado.",
        "{actor} firma contrato marco D0902 millonario. {prev_leader} al segundo puesto.",
        "{actor} captura el LER más rentable de la temporada. {prev_leader} se queda con migas.",
        "{actor} se queda con la cartera de oleosos R1208. {prev_leader}, a buscar otra cosa.",
        "Auditor ambiental designa a {actor} referente. {prev_leader} a segundo plano.",
    ],
    ...
    ("new_last", "residuos"): [
        ...
        "{actor}, código LER por confirmar. {prev_last} aclarado.",
        "{actor} hereda el expediente sancionador de {prev_last}.",
        "{actor} al periodo de prueba forzoso. {prev_last} renovado.",
        # ===== NUEVAS =====
        "{actor} olvidó actualizar SADER. {prev_last} al fin tramitó.",
        "Material no clasificable para {actor}. {prev_last} consigue salir.",
        "{actor} hereda los lodos D0901 sin caracterizar de {prev_last}.",
        "{actor}, residuo HP rechazado en planta. {prev_last}, libre.",
        "{actor} con stock vencido en almacén. {prev_last} consigue retirar.",
    ],
}
